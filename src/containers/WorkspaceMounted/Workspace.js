...
class WorkSpace extends React.Component {
  ...
  setThemeStyleFromPrefs() {
    service.api.readConfKey("prefs").then((value) => {
      if (value.interfaceStyle) {
        ...
        this.setState({
          style: require("../../app-ui-styles/quiqr10/style-" +
            themeStyle +
            ".js"),
        });
      }
  ...
  toolbarItemsLeft(siteKey, workspaceKey, activeButton) {
    return [
      <ToolbarButton
        ...
        action={() => {
          service.api.redirectTo(
            `/sites/${siteKey}/workspaces/${workspaceKey}`
          );
        }}
        title="Content"
      ...
      <ToolbarButton
        ...
        action={() => {
          service.api.redirectTo(
            `/sites/${siteKey}/workspaces/${workspaceKey}/sync/`
          );
        }}
        title="Sync"
        icon={SyncIcon}
      ...
      this.props.applicationRole === "siteDeveloper" ? (
        <ToolbarButton
          ...
          action={() => {
            service.api.redirectTo(
              `/sites/${siteKey}/workspaces/${workspaceKey}/siteconf/general`
            );
          }}
          title="Tools"
          icon={BuildIcon}
  ...
  showPreviewSiteButton() {
    if (
      ...
      this.state.workspace.serve &&
      this.state.workspace.serve[0].hugoHidePreviewSite
    ) {
      return null;
    }
  ...
  openPreviewInBrowser() {
    ...
    service.api.getCurrentBaseUrl().then((path) => {
      ...
      window
        .require("electron")
        .shell.openExternal("http://localhost:13131" + currentBaseUrlPath);
    });
  }
  ...
  renderTopToolbarRightSwitch() {
    return (
      <Routes>
        <Route
          path="/sites/:site/workspaces/:workspace/siteconf"
          render={({ match }) => {
            ...
            return (
              <TopToolbarRight
                ...
                itemsLeft={this.toolbarItemsLeft(
                  siteKey,
                  workspaceKey,
                  "tools"
                )}
        ...
        <Route
          ...
          render={({ match }) => {
            const siteKey = decodeURIComponent(match.params.site);
            const workspaceKey = decodeURIComponent(match.params.workspace);
            const toolbarItemsCenter = [this.showPreviewSiteButton()];
    ...
    return (
      <Routes>
          ...
          render={({ match }) => {
            return (
              <SyncSidebar
                ...
                workspaceKey={decodeURIComponent(match.params.workspace)}
                hideItems={
                  !this.state.forceShowMenu && !this.state.menuIsLocked
                }
                menuIsLocked={this.state.menuIsLocked}
                onToggleItemVisibility={() => {
                  this.toggleForceShowMenu();
        ...
        <Route
          path="/sites/:site/workspaces/:workspace"
          render={({ match }) => {
            return this.renderWorkspaceSidebar(
              history,
              match.url,
              match.params.site,
  ...
  renderContentSwitch() {
    return (
      <Routes>
        ...
        <Route
          path="/sites/:site/workspaces/:workspace/home/:refresh"
          exact
          render={({ match }) => {
            return this.renderDashboard(match);
          }}
        ...
        <Route
          path="/sites/:site/workspaces/:workspace/collections/:collection/:item/:refresh"
          exact
          render={({ match }) => {
            return (
              <CollectionItem
        ...
        <Route
          path="/sites/:site/workspaces/:workspace/singles/:single/:refresh"
          exact
          render={({ match }) => {
            return (
              <Single
