...
describe('Fresh Install Integration', () => {
  ...
  describe('Initial State', () => {
    ...
    it('should provide default preferences without config files', () => {
      ...
      // Only interfaceStyle is a user preference in simplified architecture
      expect(prefs.interfaceStyle).toBe('quiqr10-light');

      // dataFolder is an instance setting
  ...
  describe('User Configuration', () => {
    it('should save and retrieve user preferences', async () => {
      ...
      // Verify file was created (ELECTRON userId in simplified architecture)
      expect(existsSync(join(tempDir, 'user_prefs_ELECTRON.json'))).toBe(true);
    });
